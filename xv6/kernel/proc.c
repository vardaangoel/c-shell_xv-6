...
// CHANGES START
#if defined(SCHEDULER_MLFQ) || defined(SCHEDULER_FCFS)
#define MLFQ_LEVELS 4
static uint64 next;
static struct spinlock sequence_lock;
#endif

#ifdef SCHEDULER_MLFQ
const int slices[MLFQ_LEVELS] = {1, 2, 8, 16};
#endif
// CHANGES END
...
#if defined(SCHEDULER_MLFQ) || defined(SCHEDULER_FCFS)
  initlock(&sequence_lock, "sequence");
#endif
  for (p = proc; p < &proc[NPROC]; p++) {
...
#if defined(SCHEDULER_MLFQ) || defined(SCHEDULER_FCFS)
static void
mlfq_enqueue_locked(struct proc *p)
{
  acquire(&sequence_lock);
  p->sequence = next++;
  release(&sequence_lock);
}

static void
enqueue_locked(struct proc *p)
{
  mlfq_enqueue_locked(p);
}
...
scheduler(void)
{
  ...
  for (;;) {
    ...
    // CHANGES START
#ifdef SCHEDULER_MLFQ
    for (int queue = 0; queue < MLFQ_LEVELS && !found; queue++) {
      struct proc *selected = 0;
      uint64 selected_seq = (uint64)-1;
      for (p = proc; p < &proc[NPROC]; p++) {
        acquire(&p->lock);
        if (p->state == RUNNABLE && p->queue == queue &&
            p->sequence < selected_seq) {
          if (selected != 0)
            release(&selected->lock);
          selected = p;
          selected_seq = p->sequence;
          continue;
        }
        release(&p->lock);
      }
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if (p->state == RUNNABLE && p->sequence < selected_seq) {
        if (selected != 0)
          release(&selected->lock);
        selected = p;
        selected_seq = p->sequence;
...
yield(void)
{
  ...
  // CHANGES START
#ifdef SCHEDULER_MLFQ
  if (p->slice_ticks >= slices[p->queue]) {
    if (p->queue < MLFQ_LEVELS - 1)
      p->queue++;
    p->slice_ticks = 0;
  }
  enqueue_locked(p);
...
int
getschedstats(uint64 addr)
{
  // CHANGES START
  struct proc *p = myproc();
  struct schedstats stats;

  stats.arrival = p->arrival;
  stats.first_run = p->first_run;
  stats.ticks = p->ticks;
  stats.waiting = p->waiting;
#ifdef SCHEDULER_MLFQ
  stats.queue = p->queue;
#else
  stats.queue = -1;
#endif
  return copyout(p->pagetable, p->sz, addr, (char *)&stats, sizeof(stats));
}
// CHANGES END
...
// CHANGES START
#ifdef SCHEDULER_MLFQ
    printk(" q=%d slice=%d seq=%ld boost=%d", p->queue, p->slice_ticks,
           p->sequence, p->lastboost);
#endif
// CHANGES END
